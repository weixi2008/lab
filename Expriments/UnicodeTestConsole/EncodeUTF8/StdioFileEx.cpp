...
/*static*/bool CStdioFileEx::isFileUTF8( const CString& sFilePath )
{
	...
	if (file.Open(sFilePath, CFile::typeBinary | CFile::modeRead, &exFile))
	{
		...
		if (file.Read(firstBytes, sizeof(firstBytes)) == 3)
		{
			if(firstBytes[0] == UTF8_BOM[0] && firstBytes[1] == UTF8_BOM[1] && firstBytes[2] == UTF8_BOM[2])
			{
				bIsUTF8 = true;
			}
		}
...
/*static*/bool CStdioFileEx::isFileAnsi(const CString& sFilePath)
{
	if(!IsFileUnicode(sFilePath) && !isFileUTF8(sFilePath))
	{
		return true;
	}
	else
